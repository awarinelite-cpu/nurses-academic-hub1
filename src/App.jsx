...
const initData = async () => {
  try {
    // Check if already seeded — if so, skip all 15 reads and return instantly
    const seedSnap = await getDoc(doc(_db, "store", "_seeded"));
    if (seedSnap.exists()) {
      // Already seeded: load only critical keys needed before auth, rest load lazily
      const criticalKeys = ["nv-users", "nv-classes", "nv-announcements"];
      await Promise.all(criticalKeys.map(async key => {
        if (_cache[key] !== undefined) return;
        const snap = await getDoc(doc(_db, "store", key));
        if (snap.exists()) _notify(key, snap.data().value);
        else _notify(key, DEFAULTS_MAP[key] ?? []);
      }));
      // Load remaining keys in the background without blocking
      Promise.all(
        ALL_KEYS.filter(k => !criticalKeys.includes(k)).map(async key => {
          if (_cache[key] !== undefined) return;
          const snap = await getDoc(doc(_db, "store", key));
          if (snap.exists()) _notify(key, snap.data().value);
          else _notify(key, DEFAULTS_MAP[key] ?? []);
    ...
    // First ever load: seed all keys, then mark as seeded
    ...
    // Mark as seeded so future loads are instant
    await setDoc(doc(_db, "store", "_seeded"), { value: true, at: new Date().toISOString() });
  } catch (e) {
